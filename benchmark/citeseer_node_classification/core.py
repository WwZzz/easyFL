...
class TaskGen(BasicTaskGen):
    def __init__(self, dist_id, num_clients = 1, skewness = 0.5, local_hld_rate=0.2, seed=0):
        super(TaskGen, self).__init__(benchmark='citeseer_node_classification',
                                      dist_id=dist_id,
                                      skewness=skewness,
                                      rawdata_path='./benchmark/RAW_DATA/CITESEER',
    ...
    def load_data(self):
        self.all_data = CitationFull(root=self.rawdata_path, name='CiteSeer')
...
class TaskPipe:
    ...
    def load_task(cls, task_path, cross_validation=False):
        with open(os.path.join(task_path, 'data.json'), 'r') as inf:
            ...
            all_data = CitationFull(root='./benchmark/RAW_DATA/CITESEER', name='CiteSeer')
        # reset node masks
